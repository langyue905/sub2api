...
原始生产跟踪仅保存在本地；这里没有发布客户内容、密钥、服务器登录信息或原始内存转储。

## 独立分支上的候选修复

用户随后要求继续在独立分支做应用级修复验证。现在在 `httpUpstreamService.Do` 和 `DoWithTLS` 中为每个请求派生独立可取消上下文，并由响应体的 Close 持有其生命周期：先 cancel，再调用底层 Close，最后释放 inFlight；请求发送失败也释放该上下文。并发重复 Close 只执行一次底层关闭与计数释放。

修复没有根据 SSE completed 事件提前截断响应，没有取消调用方上下文，没有关闭全局连接复用。正常读取到 EOF 后关闭仍保留连接复用；网关原本为计费脱离客户端取消的上下文继续有效。

新增 `http_upstream_body_cancel_test.go`，直接走应用 repository 和真实 HTTP Transport：

- 显式控制 socket Read、并发 Close、上游 EOF 的先后顺序。旧代码在 2 秒保护期限内不能退出，报 `reader stranded at HTTP EOF after concurrent Close`；修复后立即完成，不依赖概率性压力复现。
- 正常完整响应保留 completed 之后的尾部数据，连续请求复用同一 HTTP/1 连接。
- HTTP/1、HTTP/2 与 DoWithTLS 生命周期入口提前关闭均可中断读流，释放在途计数且不取消父上下文。TLS 入口测试注入可信本地 client，不宣称验证了真实 uTLS 指纹握手。
- HTTP/1 和 HTTP/2 两条并发响应中，八个重复 Close 只释放一个请求；另一条响应在客户端断开后仍读取完整 usage 尾部。
- 发送失败释放派生上下文，保留调用方上下文。

本机新增测试连续运行 5 次通过，后端全量单元测试通过。GitHub CI 新增这些测试的 Linux `-race` 三次运行；其最终结果见 PR checks。生产仍未部署，候选修复不能视为生产延迟已经消失。
