...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      t('usage.time'), t('admin.usage.username'), t('usage.apiKeyFilter'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        log.created_at, log.user?.username?.trim() || log.user?.email?.trim() || '', log.api_key?.name || '', log.account?.name || '', log.model,
